...
<?php if ($this->session->userdata('user_id') == $class_details['owner_id'] && $this->session->userdata('role_id') == 1) : ?>
<script type="text/javascript">
window.onload = () => {
    const api = new JitsiMeetExternalAPI("8x8.vc", {
        roomName: "vpaas-magic-cookie-36cdf099c8cc4ba1b4d57977db5d15d1/<?php echo $class_details["room_name"]; ?> - <?php echo $class_details['class_title']; ?>",
        width: '100%',
        height: '100%',
        ...
        userInfo: {
            moderator: true,
            email: '<?php echo $class_details['user_email']; ?>',
            displayName: '<?php echo $class_details['user_name']; ?>'
        },
...
api.addEventListener('participantRoleChanged', function(event) {
    if (event.role === "moderator") {
        api.executeCommand('password', '<?php echo $class_details['pass']; ?>');
    }
});

api.executeCommand('subject', '<?php echo $class_details['class_title']; ?>');
