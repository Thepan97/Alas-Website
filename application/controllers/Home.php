...
public function save_reg() {
    ...
    $email = $data['email'];
    if ($this->Page_model->email_exists($email)) {
        $this->session->set_flashdata('error', 'Email already exists');
        redirect('page/registration-form');
        return;
    }

    // Encode specific fields to JSON
    ...
    // Sibling logic
    $data['is_siblings'] = $this->input->post('is_siblings') === 'yes' ? 1 : 0;

    if ($data['is_siblings'] === 1) {
    ...
    // Insert into DB
    $insert = $this->Page_model->insert_user($data);
 ...
 public function save_teacher()
{
    ...
    // File upload config
    $upload_path = FCPATH . 'uploads/document/';
    ...
    if (!empty($_FILES['documents_path']['name'])) {
        $file1 = $_FILES['documents_path'];
        $ext1 = pathinfo($file1['name'], PATHINFO_EXTENSION);
        $size1 = $file1['size'];

        if (!in_array(strtolower($ext1), $allowed_types)) {
            $this->session->set_flashdata('error', 'Invalid file type for Document 1. Only PDF, CSV, XLS, XLSX allowed.');
            redirect('page/teacher-form');
            return;
        }

        if ($size1 > $max_size) {
            $this->session->set_flashdata('error', 'Document 1 exceeds max size of 5MB.');
            redirect('page/teacher-form');
            return;
        }

        $new_name1 = uniqid() . '.' . $ext1;
        $full_path1 = $upload_path . $new_name1;

        if (move_uploaded_file($file1['tmp_name'], $full_path1)) {
            $data['documents_path'] = 'uploads/document/' . $new_name1;
        } else {
            $this->session->set_flashdata('error', 'Failed to upload Document 1.');
            redirect('page/teacher-form');
            return;
        }
    }
    ...
    if (!empty($_FILES['documents2_path']['name'])) {
        $file2 = $_FILES['documents2_path'];
        $ext2 = pathinfo($file2['name'], PATHINFO_EXTENSION);
        $size2 = $file2['size'];

        if (!in_array(strtolower($ext2), $allowed_types)) {
            $this->session->set_flashdata('error', 'Invalid file type for Document 2. Only PDF, CSV, XLS, XLSX allowed.');
            redirect('page/teacher-form');
            return;
        }

        if ($size2 > $max_size) {
            $this->session->set_flashdata('error', 'Document 2 exceeds max size of 5MB.');
            redirect('page/teacher-form');
            return;
        }

        $new_name2 = uniqid() . '.' . $ext2;
        $full_path2 = $upload_path . $new_name2;

        if (move_uploaded_file($file2['tmp_name'], $full_path2)) {
            $data['documents2_path'] = 'uploads/document/' . $new_name2;
        } else {
            $this->session->set_flashdata('error', 'Failed to upload Document 2.');
            redirect('page/teacher-form');
            return;
        }
    }
    ...
    public function shopping_cart($gift_status = "")
    {
        ...
        $page_data['page_url'] =  "shopping_cart";
    ...
    public function courses()
    {
        ...
        $page_data['page_url'] =  "courses_page";
    ...
    public function course($slug = "", $course_id = "")
    {
        ...
        $page_data['page_url'] =  "course_page";
    ...
    public function my_courses()
    {
        ...
        $page_data['page_url'] =  "my_courses";
        $page_data['page_title'] = site_phrase("my_courses");
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    public function my_courses_new()
    {
        if ($this->session->userdata('user_login') != true) {
            redirect(site_url('home'), 'refresh');
        }
    ...
    public function my_messages($param1 = "", $param2 = "")
    {
        ...
        $page_data['page_url'] =  "my_messages";
    ...
    public function my_wishlist()
    {
        ...
        $page_data['page_url'] =  "my_wishlist";
    ...
    public function purchase_history()
    {
        ...
        $page_data['page_url'] =  "purchase_history";
    ...
    function faq()
    {
        ...
        $page_data['page_url'] = 'website_faq';
    ...
    function become_an_instructor()
    {
        ...
        if ($this->session->userdata('user_login') != 1) {
            ...
            if (isset($_POST) && count($_POST) > 0) :
                ...
                $page_data['page_url'] = 'become_a_instructor';
