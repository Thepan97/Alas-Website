...
class Login extends CI_Controller
{
    ...
    //new function for validate_login
    public function new_validate_login($from = "")
{
    if ($this->crud_model->check_recaptcha() == false && get_frontend_settings('recaptcha_status') == true) {
        $this->session->set_flashdata('error_message', get_phrase('recaptcha_verification_failed'));
        redirect(site_url('login'), 'refresh');
    ...
    // Checking login credential from users table
    $query = $this->db->get_where('users', $credential);

    if ($query->num_rows() > 0) {
        $row = $query->row();
        // $this->user_model->new_device_login_tracker($row->id); // Optional
        $this->user_model->new_set_login_userdata($row->id);
    } else {
        $this->session->set_flashdata('error_message', get_phrase('invalid_login_credentials'));
        redirect(site_url('login'), 'refresh');
    }
 ...
 public function student_register()
    {
        ...
        $data['child_first_name'] = html_escape($this->input->post('child_first_name'));
        $data['child_last_name']  = html_escape($this->input->post('child_last_name'));
        ...
        if (empty($data['first_name']) || empty($data['last_name']) || empty($data['email']) || empty($data['password']) ||  empty($data['phone']) || empty($data['address']) || empty($data['child_first_name']) || empty($data['child_last_name']) ) {
            ...
            if(get_settings('allow_instructor')):
                if(isset($_POST['instructor']) && $_POST['instructor'] == 'yes'){
                    ...
                    if(empty($_FILES['document']['name'])){
                        $this->session->set_flashdata('error_message', get_phrase('Please choose your document file'));
                        redirect(site_url('sign_up'), 'refresh');
                    }
                    ...
                    if (in_array(strtolower($ext), $accepted_ext)) {
                        ...
                        $this->session->set_flashdata('error_message', get_phrase('Invalid document file'));
                        redirect(site_url('sign_up'), 'refresh');
                    }
                }
            endif;
    ...
    public function forgot_password_request()
    {
        ...
        $page_data['page_url'] = 'forgot_password';
